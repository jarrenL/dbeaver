...
> 2026-09-21 工行范围更正：PL/SQL 调试及 Package 属于 O（Oracle）兼容模式；M 只验收基础功能，M 调试/包编译不在需求内。当前开发基线为 26.1.5，详见 [最新范围与实现核对](ICBC_REQUIREMENTS_SCOPE_20260921.md)。下文的 26.2.0 基线及测试数字为历史记录，不代表新客户端全部重新验收。
